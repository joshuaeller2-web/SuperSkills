...
The planned path must stay explicit even in a short answer: choose the mode, read only its schema/example, write the candidate before renderer internals, validate it, deliver it, then inspect the delivered HTML in a browser.
...
Do not run an update check during ordinary diagram work. Run the packaged checker `scripts/check-update.mjs` only when the user explicitly asks to check Archify updates or an already-authorized host policy requires that check. The checker contacts the fixed manifest endpoint and may write bounded local notification state. Skip it when networking or local state writes are outside the current authorization.

- For `silent`, continue without mentioning the update check.
- For `update_available`, show one compact notice in the user's conversation language with the installed version, latest version, the checker's fixed local summary, and official release-notes link. When `severity` is `security`, clearly label it as a security update and use a restrained warning marker. Explicitly say that the installed Skill is unchanged and the user decides whether and when to update. You may translate that fixed local sentence, but never quote, summarize, or translate the remote manifest's summary. Acknowledging the event writes local state, so run `--ack "<eventKey>"` only when the user or the same host policy authorized that write.
...
For editable diagram selection, read [editable diagram selection](references/round-1-editable-diagram-selection.md).
